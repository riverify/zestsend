...
export class P2PConnection {
  ...
  handleConnection(conn, isTurnConnection = false) {
    ...
    conn.on("open", () => {
      ...
      // 新增：连接建立后，发送连接信息，通知对方连接类型
      if (this.connection && this.connection.open) {
        try {
          this.connection.send({
            type: 'connection-info',
            usingTurnRelay: this.isTurnConnection, 
            timestamp: Date.now(),
  ...
  handleData(data) {
    ...
    if (typeof data === "object" && data !== null) {
      ...
      // 新增：处理连接信息数据包，同步TURN状态
      if (data.type === 'connection-info') {
        if (data.usingTurnRelay) {
          this.logEvent("对方通过TURN服务器进行连接");
          this.isTurnConnection = true; // 将我们自己的状态也标记为使用TURN
          
          // 如果对方提供了TURN服务器信息，更新我们的TURN服务器状态
          if (data.turnServer) {
            if (this.activeTurnServer) {
              // 更新我们的TURN服务器状态，与对方保持一致
              this.activeTurnServer = {
                ...this.activeTurnServer,
                status: data.turnServer.status,
                active: true
              };
              // 通知TURN服务器状态变化
              this._notifyTurnServerChange();
            } else if (data.turnServer.url) {
              // 如果我们没有TURN服务器信息，但对方有，创建一个
              this.activeTurnServer = {
                url: data.turnServer.url,
                status: "对方使用中", // 表明是对方在使用
                active: true,
                latency: data.turnServer.latency
              };
              this._notifyTurnServerChange();
            }
          }
  ...
  // 新增：主动同步TURN服务器状态，适用于连接建立后再次确认状态
    ...
    if (this.connection && this.connection.open) {
      try {
        this.connection.send({
          ...
          turnServer: this.activeTurnServer ? {
            url: this.activeTurnServer.url,
            status: this.activeTurnServer.active ? "已连接" : this.activeTurnServer.status,
            active: this.activeTurnServer.active,
            latency: this.activeTurnServer.latency
          } : null
        });
      } catch (error) {
