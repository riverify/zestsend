...
export class P2PConnection {
      ...
      try {
        ...
        try {
          ...
          const turnServersToTest = [
            ...
            { urls: "turn:global.turn.twilio.com:3478?transport=udp", username: "f4b4035eaa76f84a0e7b6c41", credential: "t1/D5tGbJHs1jq5iN/JK/C5kPAM=" },
  ...
  connect(remotePeerId) {
    ...
    // 检查是否应该使用TURN服务器
    const shouldUseTurn = this.connectionAttempts > this.maxDirectAttempts && 
                         this.activeTurnServer && 
    ...
    if (shouldUseTurn) {
      this.logEvent(`直接连接尝试失败 ${this.maxDirectAttempts} 次，尝试使用TURN服务器`);
      ...
      const turnConfig = {
        iceServers: [
          // 包含原有的STUN服务器
          ...(this.activeStunServer ? [{ urls: this.activeStunServer.url }] : []),
          // 添加TURN服务器
          {
      ...
      this.activeTurnServer.status = "正在连接中";
      this._notifyTurnServerChange();
      ...
      this.peer.on("error", (err) => {
        this.logEvent(`使用TURN服务器时发生错误: ${err.type} - ${err.message}`, "error");
      });
    ...
    // 如果上述条件都不满足，创建新连接
    this.logEvent(`正在连接到对方: ${remotePeerId}${shouldUseTurn ? ' (通过TURN服务器)' : ''}`);
    ...
    return new Promise((resolve, reject) => {
      ...
      conn.on("open", () => {
        clearTimeout(timeout);
        this.logEvent("已成功" + (shouldUseTurn ? "通过TURN服务器" : "") + "连接到对方");
  ...
  handleConnection(conn, isTurnConnection = false) {
    ...
    this.logEvent(`建立了新连接: ${conn.peer}${isTurnConnection ? ' (通过TURN服务器)' : ''}`);
    ...
    conn.on("open", () => {
      this.logEvent(`连接打开: ${conn.peer}${isTurnConnection ? ' (通过TURN服务器)' : ''}`);
  ...
  // 新增：判断当前连接是否使用TURN中继
  isUsingTurnRelay() {
    return this.usingTurnServer && this.isTurnConnection;
  }
}
