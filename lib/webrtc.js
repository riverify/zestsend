...
export class P2PConnection {
  ...
  connect(remotePeerId) {
    ...
    const conn = this.peer.connect(remotePeerId, {
      reliable: true,
      serialization: "binary", // 改为binary，支持二进制数据传输
          ...
          setTimeout(() => {
            try {
              ...
              const progress = Math.min(
                ((chunkIndex + 1) / this._fileTransfers[fileId].totalChunks) *
                  100,
                100
              );
                ...
                setTimeout(() => {
                  ...
                  try {
                    ...
                    const totalSentBytes =
                      this._fileTransfers[fileId].sentBytes;
                    const speedMBps =
                      totalSentBytes / (1024 * 1024) / transferTime;

                    this.logEvent(
                      `文件发送完成: ${file.name} (速度: ${speedMBps.toFixed(
                        2
                      )} MB/s, 总共: ${this.formatBytes(
                        totalSentBytes
                      )}, 使用块大小: ${currentChunkSize}字节)`
                    );
