      ...
      try {
        ...
        try {
          ...
          if (successfulServers.length > 0) {
            const fastestServer = successfulServers[0];
            this.logEvent(
            ...
            iceServers = [
              { urls: fastestServer.stunUrl },
              ...iceServers.filter(
                (server) => server.urls !== fastestServer.stunUrl
              ),
            ];
          ...
          this.logEvent(`测试STUN服务器失败: ${error.message}, 使用默认服务器`);
          console.error("Error testing STUN servers:", error);
        }
