...
export class P2PConnection {
  ...
  handleData(data) {
    ...
    if (typeof data === 'object' && data !== null) {
      ...
      if (data.type === 'file-chunk' && data.fileId && data.chunkIndex !== undefined) {
        ...
        try {
          ...
          if (Array.isArray(chunkData)) {
            ...
            this.logEvent(`无法处理的文件块数据类型: ${typeof chunkData}`, 'error');
            return;
          }
          ...
          if (chunkIndex === 0 || chunkIndex % 50 === 0) {
            this.logEvent(`接收到文件块 #${chunkIndex}, 大小: ${processedData.byteLength} 字节`);
          }
      ...
      this.logEvent(`接收到二进制数据，但新的传输逻辑不需要处理此类数据。数据大小: ${data.byteLength || 'unknown'} 字节`, 'warn');
    } else {
  ...
  reconstructFile(fileId) {
    ...
    try {
      ...
      if (missingChunks.length > 0) {
        this.logEvent(`警告: 缺少 ${missingChunks.length} 个数据块，尝试继续重建`, 'warn');
      }
